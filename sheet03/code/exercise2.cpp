...
{   
    ...
    //T is the tridiagonal matrix, Q contains the Lanczos basis vectors q_i
...
int main()
{
    ...
    outfile.open("build/ex02_energy.log", std::fstream::out);
        outfile << "#e" << "\t" << "E_0" << "\n";
    outfile.close();
    ...
    for (int i = 0; i < steps; i++)
    {
        ...
        outfile.open("build/ex02_energy.log", std::fstream::app);
            outfile << e << "\t" << E_0 << "\n";
        outfile.close();
    ...
    outfile.open("build/ex02_groundstate.log", std::fstream::out);
        outfile << G;
    outfile.close();
